...
def main():
    ...
    data_clean_no_categorised_csv = inverse_categorical(data_csv, 'property_type')
    ...
    if submit:
        ...
        if not filtered_data.empty:
            ...
            for index, row in filtered_data.head(10).iterrows():
                property_type =  inverse_categorical(data_csv, 'property_type')[str(row['property_type'])]
                name = row['name']
                price = row['log_price']
                currency = '$'
